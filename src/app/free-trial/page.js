    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch('/api/demo-request', {
                ...
                body: JSON.stringify({
                    name: `${formData.parentName}${formData.childName ? ' (Child: ' + formData.childName + ', Age: ' + (formData.age || 'N/A') + ')' : ''}`,
                    email: formData.email || '',
                    phone: formData.phone || null,
                    message: [
                        formData.country ? `Country: ${formData.country}` : '',
                        formData.state ? `State: ${formData.state}` : '',
                        formData.pastTraining ? `Past Training: ${formData.pastTraining}` : '',
                        formData.message || '',
                    ].filter(Boolean).join(' | '),
                }),
            });
                    ...
                    <div className="flex flex-col lg:flex-row gap-8 xl:gap-12 w-full shadow-2xl rounded-3xl overflow-hidden relative border border-black/5 dark:border-white/5">
                        ...
                        <div className="lg:w-5/12 aspect-square lg:aspect-auto bg-[#2B3AA0] text-white p-8 sm:p-12 xl:p-16 flex flex-col justify-between relative overflow-hidden">
                            ...
                            <div className="absolute inset-0 z-0">
                                <img loading="lazy" src={media.img_free_trial_bg || "/assets/home/Practice time.jpeg"} alt="Academy Background" className="w-full h-full object-cover opacity-20 mix-blend-luminosity scale-110" />
