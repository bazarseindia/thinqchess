    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch('/api/contact', {
                ...
                body: JSON.stringify({
                    name: `${formState.parentName}${formState.childName ? ' (Child: ' + formState.childName + ')' : ''}`,
                    email: formState.email || '',
                    phone: formState.phone || null,
                    subject: formState.age ? `Age: ${formState.age}` : null,
                    message: `Country: ${formState.country || 'N/A'}, State: ${formState.state || 'N/A'}`,
                }),
            });
