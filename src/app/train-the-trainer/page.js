...
const TrainTheTrainer = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch('/api/train-the-trainer', {
                method: 'POST',
                ...
                body: JSON.stringify({
                    ...
                    experience: [
                        formData.experienceYears ? `Experience: ${formData.experienceYears}` : '',
                        formData.rating ? `Peak Rating: ${formData.rating}` : '',
                        formData.location ? `Location: ${formData.location}` : '',
                        formData.fideId ? `FIDE ID: ${formData.fideId}` : '',
                    ].filter(Boolean).join(' | '),
                    message: formData.tournamentHistory || null,
                }),
            });
            ...
            console.error('Train-the-trainer submit error:', err);
        } finally {
        ...
        <div className={`min-h-screen pt-20 ${isDark ? 'bg-[#0A1128] text-white' : 'bg-white text-[#2B3AA0]'}`}>
            ...
            <section className="relative py-24 lg:py-40 overflow-hidden">
                <div className="absolute inset-0 z-0">
                    <img loading="lazy" src={media.img_train_trainer_bg || "/images/contact-bg.jpg"}
                        alt="Background"
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4 bg-white/5 backdrop-blur-md border border-white/10 p-2 rounded-2xl">
                                    <div className="w-14 h-14 rounded-xl overflow-hidden shadow-xl border-2 border-[#FFB31A]">
                                        <img loading="lazy" src="/images/Chiranth-cropped.jpeg" alt="Head Trainer" className="w-full h-full object-cover" />
                                    </div>
                                    <div>
                        ...
                        >
                            <div className="relative rounded-[2rem] overflow-hidden border-8 border-white/5 shadow-3xl bg-[#0A1128]">
                                <img loading="lazy" src="/images/home-banner-two.jpg" alt="Mentoring" className="w-full grayscale-[30%] group-hover:grayscale-0 transition-all duration-1000 scale-105 group-hover:scale-100" />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                        ...
                        >
                            <div className="rounded-3xl overflow-hidden border-8 border-white/5 shadow-4xl relative group">
                                <img loading="lazy" src="/images/indian-img-four.jpg" alt="Academy Training" className="w-full grayscale-[20%] hover:grayscale-0 transition-all duration-700" />
                ...
                <div className="container mx-auto px-4 sm:px-6 relative z-10">
                    <div className="flex flex-col lg:flex-row items-center gap-20">
                        ...
                        <div className="lg:w-1/2 relative">
                            ...
                            >
                                <img loading="lazy" src="/assets/home/Academy photo.jpeg" alt="Academy Vibe" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />
                        ...
                        <div className="lg:col-span-7 p-12 lg:p-24 relative">
                            <form onSubmit={handleSubmit} className="space-y-12 max-w-3xl mx-auto">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-12">
                                    <InputField label="Full Name" name="fullName" placeholder="Magnus Carlsen" icon={<User size={18} />} isDark={isDark} value={formData.fullName} onChange={handleInputChange} />
                                    <InputField label="Email Address" type="email" name="email" placeholder="magnus@worldchess.com" icon={<Mail size={18} />} isDark={isDark} value={formData.email} onChange={handleInputChange} />
                                    <InputField label="Mobile Number" name="mobile" placeholder="+91 0000 0000 00" icon={<Phone size={18} />} isDark={isDark} value={formData.mobile} onChange={handleInputChange} />
                                    <InputField label="City / Country" name="location" placeholder="Bangalore, India" icon={<MapPin size={18} />} isDark={isDark} value={formData.location} onChange={handleInputChange} />
                                    <InputField label="Peak Rating" name="rating" placeholder="e.g. 2100" icon={<Trophy size={18} />} isDark={isDark} value={formData.rating} onChange={handleInputChange} />
                                    <InputField label="Experience" name="experienceYears" placeholder="e.g. 3 Years" icon={<Calendar size={18} />} isDark={isDark} value={formData.experienceYears} onChange={handleInputChange} />
                                </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <textarea
                                            ...
                                            value={formData.tournamentHistory}
                                ...
                                <button type="submit" disabled={isSubmitting} className="w-full bg-[#FFB31A] text-[#2B3AA0] py-8 rounded-[1.5rem] font-black uppercase tracking-[0.3em] text-lg hover:shadow-[0_20px_80px_rgba(255,179,26,0.3)] transition-all flex items-center justify-center gap-6 group shadow-2xl disabled:opacity-50">
                                    {isSubmitting ? 'SUBMITTING...' : submitSuccess ? 'APPLICATION SUBMITTED!' : 'SUBMIT APPLICATION'} {!isSubmitting && !submitSuccess && <ArrowRight className="group-hover:translate-x-3 transition-transform" />}
                                </button>
                            </form>
                        </div>
                    </div>
...
const InputField = ({ label, name, type = "text", placeholder, icon, isDark, value, onChange }) => (
