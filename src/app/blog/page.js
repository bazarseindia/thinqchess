...
import { blogsData as fallbackBlogs } from '@/data/blogs';
        ...
        (async () => {
            try {
                ...
                if (d.success && d.posts?.length > 0) {
                    const mapped = d.posts.map(p => ({
                        id: p.id, slug: p.slug, title: p.title,
                        excerpt: p.excerpt || p.content?.substring(0, 150) || '',
                        date: new Date(p.createdAt).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' }),
                        readTime: `${Math.ceil((p.content?.length || 500) / 1000)} min read`,
                        category: p.category || 'General',
                        image: p.featuredImage || '/assets/home/Internal Tournament.jpeg',
                        content: typeof p.content === 'string' ? [{ type: 'paragraph', text: p.content }] : p.content,
                    }));
                    setAllBlogs(mapped);
                }
            } catch (e) { }
